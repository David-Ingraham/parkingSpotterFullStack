...
def search_cameras():
    ...
    for addr in addresses:
        try:
            # Look up the address directly in our camera data (no Google Maps API needed!)
            if addr not in camera_data:
                print(f"[WARNING] Address not found in camera data: {addr}")
                continue
            
            # Get coordinates from our existing data
            search_lat = camera_data[addr].get('latitude')
            search_lng = camera_data[addr].get('longitude')
            
            if search_lat is None or search_lng is None:
                print(f"[WARNING] No coordinates for address: {addr}")
                continue
            ...
            if nearby_cameras:
                ...
                print(f"[INFO] No cameras found near location for {addr}")
